...
export const NARRATIVE: Record<TimepointId, TimepointNarrative> = {
  t0: {
    ...
    situation:   {
      ko: '호텔 예약을 취소하면 환불 {pool.advancedOutstanding}는 확정되지만, 기존 카드에서는 영업일 3~5일 동안 쓸 수 없습니다. 같은 상황을 「즉시 환불 적용」으로 한 번 더 재생하면 취소하는 순간 잔액이 복원됩니다.',
      en: 'Cancelling the hotel booking confirms a refund of {pool.advancedOutstanding}, but on an ordinary card that money is unusable for 3-5 business days. Replay the same situation with instant refund switched on and the balance is restored the moment the booking is cancelled.',
    },
    why:         {
